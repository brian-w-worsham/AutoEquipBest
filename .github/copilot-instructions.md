...
- **`EquipmentElement` over `ItemObject`:** Always use `EquipmentElement` modified stats (`GetModifiedBodyArmor()`, `GetModifiedLegArmor()`, `GetModifiedArmArmor()`, `.Weight`, `.ItemValue`) instead of base `ItemObject.ArmorComponent` fields. The modified methods account for in-game item modifiers.
...
- **Test items:** Use `TestItemFactory` helpers (`CreateArmorItem`, `CreateWeaponItem`, `CreateSimpleItem`, `ToElement`) — never construct game objects directly in tests
